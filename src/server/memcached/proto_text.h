...
namespace cachelot {
    ...
    inline void text_protocol_handler<Sock>::handle_storage_command(cache::Command cmd, bytes arguments_buf) {
        ...
        tie(alloc_error, new_item) = cache.create_item(key, calc_hash(key), (datalen + compression::COMPRESSION_OVERHEAD), flags, expiration, cas_unique);
        if (alloc_error) {
        ...
        super::async_receive_n(datalen + CRLF.length(),
            [=](const error_code net_error, const bytes data) noexcept {
                ...
                auto compressed_size = compression::compress(settings.misc.compression_level, data.rtrim_n(CRLF.length()), new_item->value_ptr());
                new_item->set_value_length(compressed_size);
                STAT_INCR(compression.bytes_saved, datalen - compressed_size);
    ...
    inline void text_protocol_handler<Sock>::handle_statistics_command(cache::Command cmd, bytes args_buf) {
        ...
        if (args_buf.empty()) {
            ...
            #define SERIALIZE_COMPRESSIONS_STAT(typ, name, desc) SERIALIZE_STAT(compression, typ, name, desc)
            COMPRESSION_STATS(SERIALIZE_COMPRESSIONS_STAT)
            #undef SERIALIZE_COMPRESSIONS_STAT
