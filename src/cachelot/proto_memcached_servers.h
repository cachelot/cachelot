...
namespace cachelot {
    ...
    public:
        text_tcp_server(io_service & ios, cache::AsyncCacheAPI & the_cache)
            : super(ios)
            , cache(the_cache) {
        }
        
        tcp_text_protocol_handler * new_connection() {
            return tcp_text_protocol_handler::create(ios(), cache);
        }
    ...
    public:
        explicit text_unix_server(asio::io_service & ios, cache::AsyncCacheAPI & the_cache)
            : super(ios)
            , cache(the_cache) {
        }
