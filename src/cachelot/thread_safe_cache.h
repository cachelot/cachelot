...
#define CACHELOT_THREAD_SAFE_CACHE_H_INCLUDED
...
#undef CACHE_REQUESTS_ENUM_ELEMENT_TYPE
        ...
        public:
            /// @copydoc Cache::Cache()
            explicit ThreadSafeCache(size_t memory_size, size_t initial_dict_size);
            ...
            /// just call passed `callback` function in background thread processing queue
            /// all requests are ordered, so delayed call acknowledges completion of certain sequence
            template <typename Callback>
            void delayed_call(Callback callback) noexcept;
        ...
        struct ThreadSafeCache::RequestArgs {
            /// requested item key
            const bytes key;
            /// requested item hash
        ...
        struct ThreadSafeCache::GetRequestArgs : public RequestArgs {
        ...
        struct ThreadSafeCache::StoreRequestArgs : public RequestArgs {
        ...
        struct ThreadSafeCache::DelRequestArgs : public RequestArgs {
            /// callback to notify operation completion
        ...
        struct ThreadSafeCache::TouchRequestArgs : public RequestArgs {
            /// expiration time point
            seconds expires;
        ...
        struct ThreadSafeCache::SyncRequestArgs {
            /// callback
        ...
        struct ThreadSafeCache::AsyncRequest : public mpsc_queue<ThreadSafeCache::AsyncRequest>::node {
            /// type of request
            const Request type;
        ...
        template <typename Callback>
        inline void ThreadSafeCache::do_get(const bytes key, const hash_type hash, Callback on_get) noexcept {
        ...
        template <typename Callback>
        inline void ThreadSafeCache::do_store(const Request rtype, const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_store) noexcept {
            debug_assert(not m_terminated);
            try {
        ...
        template <typename Callback>
        inline void ThreadSafeCache::do_set(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_set) noexcept {
            do_store(SET, key, hash, value, flags, expires, cas_value, on_set);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_add(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_add) noexcept {
            do_store(ADD, key, hash, value, flags, expires, cas_value, on_add);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_replace(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_replace) noexcept {
            do_store(REPLACE, key, hash, value, flags, expires, cas_value, on_replace);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_cas(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_cas) noexcept {
            do_store(CAS, key, hash, value, flags, expires, cas_value, on_cas);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_append(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_append) noexcept {
            do_store(APPEND, key, hash, value, flags, expires, cas_value, on_append);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_prepend(const bytes key, const hash_type hash, bytes value, opaque_flags_type flags, seconds expires, cas_value_type cas_value, Callback on_prepend) noexcept {
            do_store(PREPEND, key, hash, value, flags, expires, cas_value, on_prepend);
        }


        template <typename Callback>
        inline void ThreadSafeCache::do_del(const bytes key, const hash_type hash, Callback on_del) noexcept {
            debug_assert(not m_terminated);
            try {
        ...
        template <typename Callback>
        inline void ThreadSafeCache::do_touch(const bytes key, const hash_type hash, seconds expires, Callback on_touch) noexcept {
            debug_assert(not m_terminated);
            try {
        ...
        template <typename Callback>
        inline void ThreadSafeCache::delayed_call(Callback callback) noexcept {
            debug_assert(not m_terminated);
            try {
...
#endif // CACHELOT_THREAD_SAFE_CACHE_H_INCLUDED
